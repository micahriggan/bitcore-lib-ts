...
export class Interpreter {
  ...
  public verifyWitnessProgram(version, program, witness, satoshis, flags) {
    ...
    if (version === 0) {
      if (program.length === 32) {
        ...
        scriptPubKey.add(OP_CODES.OP_DUP);
        scriptPubKey.add(OP_CODES.OP_HASH160);
        scriptPubKey.add(program);
        scriptPubKey.add(OP_CODES.OP_EQUALVERIFY);
        scriptPubKey.add(OP_CODES.OP_CHECKSIG);
      ...
      if (scriptPubkey.isWitnessProgram(witnessValues)) {
        ...
        if (
          !this.verifyWitnessProgram(
            witnessValues.version,
            witnessValues.program,
            witness,
            satoshis,
            flags
          )
  ...
  public static true = Buffer.from([1]);
  public static false = Buffer.from([]);
  ...
  public static SCRIPT_VERIFY_NONE = 0;

  // Making v1-v16 witness program non-standard
  public static SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_WITNESS_PROGRAM = 1 << 12;

  // Evaluate P2SH subscripts (softfork safe, BIP16).
  public static SCRIPT_VERIFY_P2SH = 1 << 0;

  // Passing a non-strict-DER signature or one with undefined hashtype to a checksig operation causes script failure.
  // Passing a pubkey that is not (0x04 + 64 bytes) or (0x02 or 0x03 + 32 bytes) to checksig causes that pubkey to be
  // skipped (not softfork safe: this flag can widen the validity of OP_CHECKSIG OP_NOT).
  public static SCRIPT_VERIFY_STRICTENC = 1 << 1;

  // Passing a non-strict-DER signature to a checksig operation causes script failure (softfork safe, BIP62 rule 1)
  public static SCRIPT_VERIFY_DERSIG = 1 << 2;

  // Passing a non-strict-DER signature or one with S > order/2 to a checksig operation causes script failure
  // (softfork safe, BIP62 rule 5).
  public static SCRIPT_VERIFY_LOW_S = 1 << 3;

  // verify dummy stack item consumed by CHECKMULTISIG is of zero-length (softfork safe, BIP62 rule 7).
  public static SCRIPT_VERIFY_NULLDUMMY = 1 << 4;

  // Using a non-push operator in the scriptSig causes script failure (softfork safe, BIP62 rule 2).
  public static SCRIPT_VERIFY_SIGPUSHONLY = 1 << 5;
  ...
  // (softfork safe)
  public static SCRIPT_VERIFY_MINIMALDATA = 1 << 6;
  ...
  /*
   *public static SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS = (1 << 7);
   */

  // CLTV See BIP65 for details.
  public static SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY = 1 << 9;
   ...
   */
  public checkPubkeyEncoding(buf) {
    if (
   ...
   */
  public checkLockTime(nLockTime) {
  ...
  public step() {
    ...
    // Note how OP_CODES.OP_RESERVED does not count towards the opcode limit.
    if (opcodenum > OP_CODES.OP_16 && ++this.nOpCount > 201) {
      this.errstr = 'SCRIPT_ERR_OP_COUNT';
      return false;
    }

    if (
      opcodenum === OP_CODES.OP_CAT ||
      opcodenum === OP_CODES.OP_SUBSTR ||
      opcodenum === OP_CODES.OP_LEFT ||
      opcodenum === OP_CODES.OP_RIGHT ||
      opcodenum === OP_CODES.OP_INVERT ||
      opcodenum === OP_CODES.OP_AND ||
      opcodenum === OP_CODES.OP_OR ||
      opcodenum === OP_CODES.OP_XOR ||
      opcodenum === OP_CODES.OP_2MUL ||
      opcodenum === OP_CODES.OP_2DIV ||
      opcodenum === OP_CODES.OP_MUL ||
      opcodenum === OP_CODES.OP_DIV ||
      opcodenum === OP_CODES.OP_MOD ||
      opcodenum === OP_CODES.OP_LSHIFT ||
      opcodenum === OP_CODES.OP_RSHIFT
    ) {
      this.errstr = 'SCRIPT_ERR_DISABLED_OPCODE';
      return false;
    }

    if (fExec && 0 <= opcodenum && opcodenum <= OP_CODES.OP_PUSHDATA4) {
      ...
      switch (opcodenum) {
        ...
        case OP_CODES.OP_CHECKLOCKTIMEVERIFY:
        ...
        case OP_CODES.OP_NOTIF:
          {
            ...
            if (fExec) {
              ...
              if (opcodenum === OP_CODES.OP_NOTIF) {
                fValue = !fValue;
              }
            }
        ...
        case OP_CODES.OP_EQUALVERIFY:
          //case OP_CODES.OP_NOTEQUAL: // use OP_CODES.OP_NUMNOTEQUAL
          {
            ...
            if (opcodenum === OP_CODES.OP_EQUALVERIFY) {
              if (fEqual) {
                this.stack.pop();
              } else {
        ...
        case OP_CODES.OP_MAX:
          {
            ...
            if (opcodenum === OP_CODES.OP_NUMEQUALVERIFY) {
        ...
        case OP_CODES.OP_HASH256:
          {
            ...
            } else if (opcodenum === OP_CODES.OP_SHA256) {
              bufHash = Hash.sha256(buf);
            } else if (opcodenum === OP_CODES.OP_HASH160) {
              bufHash = Hash.sha256ripemd160(buf);
            } else if (opcodenum === OP_CODES.OP_HASH256) {
        ...
        case OP_CODES.OP_CHECKSIGVERIFY:
          {
            ...
            if (opcodenum === OP_CODES.OP_CHECKSIGVERIFY) {
              if (fSuccess) {
                this.stack.pop();
              } else {
        ...
        case OP_CODES.OP_CHECKMULTISIGVERIFY:
          {
            ...
            if (opcodenum === OP_CODES.OP_CHECKMULTISIGVERIFY) {
              if (fSuccess) {
                this.stack.pop();
              } else {
