...
describe('Hash', () => {
  ...
  describe('@sha1', () => {
    it('calculates the hash of this buffer correctly', () => {
  ...
  describe('#sha256', () => {
    ...
    it('fails when the input is not a buffer', () => {
      Hash.sha256.bind(Hash, str).should.throw('Invalid Argument');
    });
  });

  describe('#sha256hmac', () => {
    it('computes this known big key correctly', () => {
    ...
    it('computes this known empty test vector correctly', () => {
      const key = new Buffer('');
      const data = new Buffer('');
      Hash.sha256hmac(data, key)
        .toString('hex')
    ...
    it('computes this known non-empty test vector correctly', () => {
  ...
  describe('#sha512hmac', () => {
    it('calculates this known empty test vector correctly', () => {
      const hex =
      ...
      Hash.sha512hmac(new Buffer([]), new Buffer([]))
        .toString('hex')
        .should.equal(hex);
    });

    it('calculates this known non-empty test vector correctly', () => {
      const hex =
      ...
      const data = new Buffer('test1');
      const key = new Buffer('test2');
