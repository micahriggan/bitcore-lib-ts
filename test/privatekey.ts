...
describe('PrivateKey', () => {
  ...
  describe('instantiation', () => {
    ...
    it('should not be able to instantiate private key WIF is too long', () => {
      expect(() => {
        const bufz = Base58Check.decode(
          'L3T1s1TYP9oyhHpXgkyLoJFGniEgkv2Jhi138d7R2yJ9F4QdDU2m'
        );
        const buf2 = Buffer.concat([bufz, new Buffer(0x01)]);
    ...
    it('should not be able to instantiate private key WIF because of network mismatch', () => {
      expect(() => {
        const a = new PrivateKey(wifNamecoin, 'testnet');
      }).to.throw('Invalid network');
    });
  ...
  describe('buffer serialization', () => {
    ...
    // it('will output a 32 byte buffer', () => {
    ...
    // TODO: enable for v1.0.0 when toBuffer is changed to always be 32 bytes long
    // it('should return buffer with length equal 32', () => {
  ...
  describe('#toPublicKey', () => {
    it('should convert this known PrivateKey to known PublicKey', () => {
      ...
      const pubhex =
        '02a1633cafcc01ebfb6d78e39f687a1f0995c62fc95f51ead10a02ee0be551b5dc';
      const privkey = new PrivateKey(new BN(new Buffer(privhex, 'hex')));
  ...
  it('creates an address as expected from WIF, livenet', () => {
    const privkey = new PrivateKey(
      '5J2NYGstJg7aJQEqNwYp4enG5BSfFdKXVTtBLvHicnRGD5kjxi6'
    );
    privkey.publicKey
      .toAddress()
      .toString()
      .should.equal('135bwugFCmhmNU3SeCsJeTqvo5ViymgwZ9');
  });
  ...
  it('creates an address as expected from WIF, testnet', () => {
    ...
    privkey.publicKey
      .toAddress()
      .toString()
      .should.equal('moiAvLUw16qgrwhFGo1eDnXHC2wPMYiv7Y');
  });
});
