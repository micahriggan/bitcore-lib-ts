...
describe('MultiSigInput', () => {
  ...
  it('can count missing signatures, signed with key 3 and 1', () => {
    const transaction = new Transaction()
      .from(output, [public1, public2, public3], 2)
      .to(address, 1000000);
    const input = transaction.inputs[0];
  ...
  it('can parse list of signature buffers, from TX signed with key 1 and 2', () => {
    const transaction = new Transaction(
    ...
    inputObj.signatures = MultiSigInput.normalizeSignatures(
      ...
      0,
      transaction.inputs[0].script.chunks.slice(1).map(s => {
        return s.buf;
      }),
