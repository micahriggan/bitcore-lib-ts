...
const TransactionSignature = BitcoreLib.Transaction.Signature;
...
describe('TransactionSignature', () => {
  ...
  it('fails when trying to create from invalid arguments', () => {
    expect(() => {
      return new (TransactionSignature as any)();
    }).to.throw(new BitcoreError(BitcoreError.Types.InvalidArgument));
    expect(() => {
      return new TransactionSignature(1 as any);
    }).to.throw(new BitcoreError(BitcoreError.Types.InvalidArgument));
    expect(() => {
      return new TransactionSignature('hello world' as any);
    }).to.throw(new BitcoreError(BitcoreError.Types.InvalidArgument));
  });
  ...
  describe('serialization', () => {
    ...
    it('can roundtrip to/from json', () => {
      ...
      expect(new TransactionSignature(JSON.parse(json)).toObject()).to.deep.equal(
        serialized
      );
      expect(
    ...
    it('can deserialize a previously known object', () => {
      expect(new TransactionSignature(testObject).toObject()).to.deep.equal(
        testObject
      );
    });
  });
