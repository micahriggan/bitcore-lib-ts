...
const UnspentOutput = BitcoreLib.UnspentOutput;
...
describe('UnspentOutput', () => {
  ...
  it('can be created without "new" operand', () => {
    expect(new UnspentOutput(sampleData1) instanceof UnspentOutput).to.equal(true);
  });
  ...
  it('fails if vout is not a number', () => {
    const sample: any = _.cloneDeep(sampleData2);
    sample.vout = '1';
    expect(() => {
