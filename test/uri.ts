...
describe('URI', () => {
  ...
  it('parses uri strings correctly (test vector)', () => {
    ...
    uri = URI.parse(
      'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=123.22' +
  ...
  it('URIs can be validated statically (test vector)', () => {
    URI.isValid('bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj').should.equal(
      true
    );
    URI.isValid('bitcoin:mkYY5NRvikVBY1EPtaq9fAFgquesdjqECw').should.equal(
      true
    );

    URI.isValid(
      'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2'
    ).should.equal(true);
    URI.isValid(
      'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&other=param'
    ).should.equal(true);
    URI.isValid(
      'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&req-other=param',
      ['req-other']
    ).should.equal(true);
    URI.isValid(
      'bitcoin:mmrqEBJxUCf42vdb3oozZtyz5mKr3Vb2Em?amount=0.1&' +
    ...
    URI.isValid(
      'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfk?amount=bad'
    ).should.equal(false);
    URI.isValid(
      'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfk?amount=1.2&req-other=param'
    ).should.equal(false);
    URI.isValid(
      'bitcoin:?r=https%3A%2F%2Ftest.bitpay.com%2Fi%2F6DKgf8cnJC388irbXk5hHu'
    ).should.equal(false);
  });
  ...
  it('do not need new keyword', () => {
    const uri = new URI('bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  ...
  describe('instantiation from bitcoin uri', () => {
    ...
    it('parses address', () => {
      ...
      uri.address.should.be.instanceof(BitcoreLib.Address);
    ...
    it('throws error when a required feature is not supported', () => {
      (() => {
        return new URI(
          'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=1.2&other=param&req-required=param'
        );
      }).should.throw(Error);
    });
  ...
  it('should support numeric amounts', () => {
    const uri = new URI(
      'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj?amount=12.10001'
    );
  ...
  it('should generate a valid URI', () => {
    new URI({
      ...
      .toString()
      .should.equal('bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  ...
  it('should be case insensitive to protocol', () => {
    const uri1 = new URI('bItcOin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
    const uri2 = new URI('bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj');
  ...
  it('displays nicely on the console (#inspect)', () => {
    const uri = 'bitcoin:1DP69gMMvSuYhbnxsi4EJEFufUAbDrEQfj';
